...
const ProgressSteps: React.ForwardRefRenderFunction<
	ProgressStepsHandles,
	ProgressStepsProps
> = ({ children }, ref) => {
		...
		<div>
			<div className="position-relative m-4">
				<div className="progress" style={{ height: '3px' }}>
					<div
						...
						role="progressbar"
						style={{ width: `${getPositionRelative(currentStep)}%` }}
					></div>
				</div>
				{children.map((_, index) => (
					<div
						key={index}
						className="position-absolute top-0 translate-middle bg-white "
						style={{ left: `${getPositionRelative(index)}%` }}
					>
						<button
			...
			<div className="pt-4">
				{children.map((child, index) => (
					<div
						key={index}
						className={`${index === currentStep ? '' : 'visually-hidden'}`}
					>
						{child}
					</div>
				))}
