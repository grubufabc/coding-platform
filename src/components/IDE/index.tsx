...
const IDE: React.ForwardRefRenderFunction<IDEHandles, IDEProps> = (
	{ onChange, classNameCodeEditor = '' },
	ref
) => {
	const { request, loading } = useFetch();

	const [stdinIDE, setStdinIDE] = React.useState<string>('');
	const [stdout, setStdout] = React.useState<StdoutState>({
		message: '',
		error: false,
	});
	...
	const handleRunCode = async () => {
		...
		const isValid = [validateCode, validateLanguage].every((fn) => fn());
