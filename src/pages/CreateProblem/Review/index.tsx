...
const Review: React.FC<ReviewProps> = ({ handleCreateProblem, description, testCases, title }) => {
    return (
        <React.Fragment>
            ...
            <div className="row">
                ...
                {testCases.filter((testCase) => testCase.visible).map((testCase, index) => (
                    <div className="mb-3" key={index}>
                        <TestCaseItem input={testCase.input} expectedOutput={testCase.expectedOutput} />
                    </div>
                ))}
            </div>
        </React.Fragment>
