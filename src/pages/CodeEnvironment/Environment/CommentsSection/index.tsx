...
const CommentsSection: React.FC<CommentsSectionProps> = ({
	...
	const [avatarColors, setAvatarColors] = React.useState<Map<string, string>>(
		...
		<div className="d-flex flex-column h-100">
			<div>
				<h4 className="mb-3">
					Comentários (
					{
						comments.filter((comment) => comment.commit_id === selectedCommitId)
							.length
					}
					)
				</h4>
				{comments.filter((comment) => comment.commit_id === selectedCommitId)
					.length === 0 && <p>Nenhum comentário</p>}
			...
			<div className="flex-grow-1">
				{comments
					.filter((comment) => comment.commit_id === selectedCommitId)
					.map((comment, index) => (
						<CommentDisplay
							key={index}
