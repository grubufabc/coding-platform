...
const Environment: React.FC = () => {
	...
	React.useEffect(() => {
		if (codeEnvironment.states.length > 0) {
			...
			if (
				commit_id &&
				codeEnvironment.states.find((state) => state.id === commit_id)
			) {
				setSelectedCommitId(commit_id);
				setCommitPath(
					getPathFromCurrentCommitToRoot(commit_id, codeEnvironment.states)
				);
			} else {
				const lastCommitId =
				...
				setCommitPath(
					getPathFromCurrentCommitToRoot(lastCommitId, codeEnvironment.states)
				);
			}
		}
		setEnvironmentName(codeEnvironment.name);
	...
	React.useEffect(() => {
		const commit = codeEnvironment.states.find(
		...
		const language = languages.find(
			(language) => language.id === commit.code.language_id
		);
		if (!language) {
			return;
		}
	...
	const handleShareEnvironment = (format: string) => {
		const formatters = new Map<string, (url: string) => string>([
			[
				'facebook',
				(url: string) => `https://www.facebook.com/sharer/sharer.php?u=${url}`,
			],
			['url', (url: string) => url],
		]);
							...
							<Sidebar.Pane id="share">
								<div>
									...
									<div className="btn-group" role="group">
										{Array.from(shareIcons.entries()).map(
											([format, Icon], index) => {
												return (
													<button
														type="button"
								...
								<div>
									...
									<CommitsSection
										...
										setSelectedCommitId={(commit_id: string) => {
											setCommitPath(
												getPathFromCurrentCommitToRoot(
													commit_id,
													codeEnvironment.states
												)
											);
